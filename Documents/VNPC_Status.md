...
**MCP status:** Installed **v0.72.0** (upgraded 2026-05-11 from 0.66.1 via the universal direct-jump recipe), connected via streamableHttp on port 52724. See `E:\Unity\Sandbox\Documents\Canonical\MCP_ConnectionBrief.md` for transport details and the cross-fleet state table.

**Unity:** 6000.3.15f1 (URP).

**Asset stacks evaluated and approved:**
- **VN stack (active in VNPC):** Naninovel + Text Animator (+ Dialogue System for cross-game shared logic)
- **P&C stack (used by GRIMMORPG / P&C Game 2 in their own projects):** Adventure Creator + Dialogue System + Ink + Text Animator
...
**Session 11 (Apr 27, 2026):** MCP migrated to streamableHttp transport. Plugin updated to v0.66.0 (from v0.48.1). Unity AI Game Developer panel: Transport flipped from stdio -> http, port unchanged at 52724. Reconfigure regenerated `.mcp.json` in URL form (`"type": "http"` + `"url": "http://localhost:52724"`). Hand-created matching `.claude/mcp.json` (Reconfigure didn't write it -- known per the migration brief). Verification passed: status connected, ping echo, scene-list-opened returns GITT scene, tool-list returns 116 tools, single `unity-mcp-server.exe` parented to Unity.exe (not Code.exe), no new bind errors since pre-restart. Port Registry in `E:\Unity\Sandbox\Documents\MCP_ConnectionBrief.md` updated for VNPC. The migration eliminates the prior port-race bug (VS Code Claude no longer spawns its own server -- it just connects to the URL the Unity-launched server is already listening on).

**Session 12 (May 11, 2026):** Post-crash recovery -- doc + memory audit. Project relocated `E:\Unity\VNPC\VisualNovelPointClick` -> `E:\Unity\VisualNovelPointClick`. Unity bumped to 6000.3.15f1. MCP plugin upgraded 0.66.1 -> 0.72.0 via the universal direct-jump recipe (Rune-altered workflow now in `MCP_ConnectionBrief.md` -- VNPC added as a verified data point alongside SS, HOK, BM, AudioProject). Project memory rebuilt at `C:\Users\steph\.claude\projects\e--Unity-VisualNovelPointClick\memory\` (8 entries: user_role, project_overview, project_mcp_state, project_crash_recovery, reference_vnpc_docs, reference_vnpc_local_gotchas, reference_tecvoodoo_coding_standards, reference_tecvoodoo_universal_workflow). Canonical adoption applied per `PerProject_CanonicalAdoption_Prompt.md`: CLAUDE.md narrowed to pointer-index shape; `VNPC_DevReference.md` rewritten with canonical pointer pattern (universal rules now point at `TecVooDoo_CodingStandards.md` + `UniversalWorkflow.md`); `VNPC_CodeReference.md` refreshed (TATextPrinterPanel entry added, MCP version updated, scope narrowed to VN stack since P&C games moved out). Uncommitted pre-crash audio reorg (27 SFX deletions + new `Resources/Naninovel/Audio/` folder + 13 modified .nani scripts) left in the working tree for user review -- not part of this audit commit.
